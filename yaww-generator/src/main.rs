...
fn main() -> Result<()> {
    ...
    let mut state = State::new(config, &mut sw_buffer)?;

    // begin header
    swwriteln!(state, "// Automatically generated by yaww-generator")?;
    swwriteln!(
        state,
    ...
    swwriteln!(state, "use cstr_core::CStr;")?;
    swwriteln!(state, r#"#[cfg(feature = "alloc")]"#)?;
    swwriteln!(state, "use alloc::vec::Vec;")?;
    swwriteln!(state, r#"#[cfg(feature = "alloc")]"#)?;
    swwriteln!(state, "use cstr_core::CString;")?;
    swwriteln!(state, r#"#[cfg(feature = "std")]"#)?;
    swwriteln!(
        state,
        "use std::{{ffi::{{OsStr, OsString}}, os::windows::prelude::*}};"
    )?;
    swwriteln!(state, "#[doc(inline)]")?;
    swwriteln!(state, "use windows_sys::core::*;")?;
    swwriteln!(
        state,
...
pub struct State<'s> {
    ...
    pub dont_merge_globs: Vec<Pattern>,
    pub stub_struct_globs: Vec<Pattern>,

    sw_output: Indented<'s, String>,
    indent: usize,
    is_comment: bool,

    constants_emitted: HashSet<&'static str>,
...
impl<'s> State<'s> {
    fn new(config: Config<'s>, sw_output: &'s mut String) -> Result<Self> {
        Ok(Self {
            ignore_globs: compile_globs(&config.directives.ignore)?,
            actual_bool_globs: compile_globs(&config.directives.actual_bools)?,
            dont_merge_globs: compile_globs(&config.directives.dont_merge)?,
            stub_struct_globs: compile_globs(&config.directives.stub_struct)?,
            config,
            ...
            counter: 0,
            constants_emitted: HashSet::new(),
        })
    }

    fn update_indent(&mut self) {
        let source = if self.is_comment { COMMENT } else { INDENT };

        let indentation = &source[..self.indent * 4];
    ...
    fn get_item(&mut self, ty: &Type) -> Result<types::Type> {
        match ty {
            ...
            Type::Char => Ok(types::Type::primitive("u16")),
            ...
            Type::TypeDef(td) => {
                ...
                if td.is_union() {
                    // we stub unions out and implement them for realsies in the
                    // crate's body
                    ...
                    for (i, field) in td.fields().enumerate() {
                        fields.push(Field {
                            ...
                            ty: ResolveLater::resolved(types::Type::Stub),
                        });
                    }
    ...
    fn compose_struct(&mut self, td: &TypeDef) -> Result<Item> {
        ...
        let real_arr_type = self
            .config
            .real_arr_type
            .get(td.name())
            .map(|rat| anyhow::Ok((Pattern::new(&rat.fname)?, rat.real_ty.clone())))
            .transpose()?;
                        ...
                        if let types::Type::Array(ref mut ty, ..) = ty {
                            ...
                            if let Some(ref arr_type) = real_arr_type {
                                if arr_type.0.matches(field.name()) {
                                    // set the type
                                    *ty = Box::new(types::Type::Primitive(Cow::Owned(
                                        arr_type.1.to_string(),
                                    )));
                                }
                            }
                        }
    ...
    fn item_for_typedef(&mut self, td: &'static TypeDef) -> Result<Option<Item>> {
        ...
        // explicitly mark wparam and lparam
        match td.name().to_lowercase().as_str() {
            "wparam" => return Ok(Some(Item::Special(Special::Wparam))),
            "lparam" => return Ok(Some(Item::Special(Special::Lparam))),
            "lresult" => return Ok(Some(Item::Special(Special::Lresult))),
            _ => {}
        }
        ...
        Ok(if td.is_handle() {
            ...
            if td
                ...
                .is_some()
            {
                // emit an import for the handle IRL so we can use it in win32 params
                if td.name() != td.name().to_upper_camel_case() {
                    swwriteln!(self, "#[allow(unused_imports)]")?;
                    swwriteln!(
                        self,
                        "use {}::{};",
                        format_namespace(self.current_namespace.unwrap()),
                        td.name()
                    )?;
                }
    ...
    fn handle_type(&mut self, ty: &'static Type) -> Result<()> {
        let (name, item) = match ty {
            Type::TypeDef(td) => {
                ...
                if fits_globs(td.name(), &self.ignore_globs) {
    ...
    fn handle_methods(&mut self) -> Result<()> {
        'l: for md in mem::take(&mut self.methods_deferred) {
            ...
            // do we ignore this
            if fits_globs(md.name(), &self.ignore_globs) {
                continue 'l;
            }
            ...
            let mut function = Function {
                name: md.name(),
                merged_params: Vec::new(),
                params,
                return_type,
                namespace: namespace.to_string(),
            };
            function.merge_params(self)?;
            function
                .write(self)
...
fn compile_globs<R: AsRef<str>>(globs: impl IntoIterator<Item = R>) -> Result<Vec<Pattern>> {
    globs
        .into_iter()
...
fn fits_globs<'a>(name: &str, globs: impl IntoIterator<Item = &'a Pattern>) -> bool {
    globs.into_iter().any(|glob| glob.matches(name))
}

fn any_res(i: impl IntoIterator<Item = Result<bool>>) -> Result<bool> {
    i.into_iter().try_fold(false, |acc, b| Ok(acc || b?))
}

fn all_res(i: impl IntoIterator<Item = Result<bool>>) -> Result<bool> {
    i.into_iter().try_fold(true, |acc, b| Ok(acc && b?))
}
